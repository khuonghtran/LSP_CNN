...
# Use Linear Interpolation to fill the nan values
features_norm_df = pd.DataFrame(features_norm)
features_norm = features_norm_df.interpolate(method='linear', axis=1, limit_direction='both').to_numpy()
...
if len(features_train.shape)<3:
    ...
    features_test_raw = np.expand_dims(features_test_raw, axis=2)
...
if __name__ == "__main__":
    ...
    for i in range(ITERS):
        ...
        importlib.reload(CNN_1D_UNet_v2)
        model = CNN_1D_UNet_v2.build_unet_1d(n_times=features_train.shape[1],n_feature=features_train.shape[2], n_out=4,units=128,drop=model_drop)  # dense_layer_n=PRE_TRAIN
        ...
        r,p_value,mad,rmse,msb,labels_testout,labels_testout_std,labels_pred,labels_pred_std,labels_pred_masked,labels_pred_std_masked,labels_pred2 = customized_train_v2.test_lsp_accuacy(model,testx_transformer,testy_transformer2,labels_min.reshape(-1,244,4),labels_max.reshape(-1,244,4),features_test_raw)
        ...
        df_features_test = pd.DataFrame(features_test_raw.reshape(-1,244),columns=colnames) #reshape to 2D array.
        ...
        model_name = MODEL_DIR+base_name+'.model.ITER'+str(i)+'.keras'
        print(model_name)    
