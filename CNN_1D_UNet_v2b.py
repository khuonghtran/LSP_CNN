...
def build_unet_1d(n_times, n_feature, n_out, units, drop=0.5):
    ...
    c4 = layers.BatchNormalization()(c4)  # Batch normalization
    c4 = layers.Dropout(drop)(c4)	

    # Decoder (expanding path)
    u5 = layers.Conv1DTranspose(filters=units*4, kernel_size=2, strides=2, padding='same')(c4)
    if u5.shape[1]<c3.shape[1]:
        # Padding configuration: dynamically retrieve first dimension size.
        padding = [[0, 0], [0, c3.shape[1]-u5.shape[1]], [0, 0]]  # Padding the second dimension by 1 at the end
        # Apply the padding
        u5 = tf.pad(u5, paddings=padding)	
		
    u5 = layers.concatenate([u5, c3])
    c5 = layers.Conv1D(filters=units*4, kernel_size=3, activation='relu', padding='same')(u5)
    c5 = layers.BatchNormalization()(c5)  # Batch normalization
    c5 = layers.Conv1D(filters=units*4, kernel_size=3, activation='relu', padding='same')(c5)
    c5 = layers.BatchNormalization()(c5)  # Batch normalization
    c5 = layers.Dropout(drop)(c5)	

    u6 = layers.Conv1DTranspose(filters=units*2, kernel_size=2, strides=2, padding='same')(c5)
    u6 = layers.concatenate([u6, c2])
    c6 = layers.Conv1D(filters=units*2, kernel_size=3, activation='relu', padding='same')(u6)
    c6 = layers.BatchNormalization()(c6)  # Batch normalization
    c6 = layers.Conv1D(filters=units*2, kernel_size=3, activation='relu', padding='same')(c6)
    c6 = layers.BatchNormalization()(c6)  # Batch normalization
    c6 = layers.Dropout(drop)(c6)

    u7 = layers.Conv1DTranspose(filters=units, kernel_size=2, strides=2, padding='same')(c6)
    u7 = layers.concatenate([u7, c1])
    c7 = layers.Conv1D(filters=units, kernel_size=3, activation='relu', padding='same')(u7)
    c7 = layers.BatchNormalization()(c7)  # Batch normalization
    c7 = layers.Conv1D(filters=units, kernel_size=3, activation='relu', padding='same')(c7)
    c7 = layers.BatchNormalization()(c7)  # Batch normalization
    ...
    outputs = layers.Conv1D(filters=n_out, kernel_size=1, activation='sigmoid')(c7)
